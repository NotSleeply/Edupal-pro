...
import QuestionDialogCp from "./QuestionDialogCp";
...
export default function QuestionList() {
  ...
  const fetchTraceData = async () => {
    ...
    try {
      ...
      // 这里增加本地 mock 读取
      if (reduxTrace.sourceId && reduxTrace.sourceId.startsWith("mock_")) {
        const mockQuestions = JSON.parse(localStorage.getItem(`questions_${reduxTrace.sourceId}`) || "[]");
        if (mockQuestions.length > 0) {
          const tempquestions = mockQuestions.map((item: any) => ({
            id: item.id,
            question: item.content,
            difficulty: item.difficulty,
            creator: item.creator,
